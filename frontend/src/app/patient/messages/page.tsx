  ...
  return (
    <div className="min-h-screen bg-cream-100">
      <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600 mt-1">Chat with your care team</p>
              </div>
            </div>
        ...
        {/* Chat Container with rounded corners and shadow */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="flex h-[calc(100vh-200px)] min-h-[600px]">
            {/* Conversations Sidebar */}
            <div className={`${
              showConversations ? 'block' : 'hidden'
            } lg:block w-full lg:w-1/3 xl:w-1/4 border-r border-slate-200 overflow-y-auto`}>
              {conversations.map(conversation => (
                <button
                  key={conversation.id}
                  onClick={() => setSelectedConversation(conversation)}
                  className={`w-full text-left p-4 hover:bg-slate-50 border-b border-slate-100 transition-colors ${
                    selectedConversation?.id === conversation.id ? 'bg-coral-50 border-l-4 border-l-coral-500' : ''
                  }`}
                >
                  <div className="flex items-start gap-3">
                    {/* Provider Avatar */}
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-coral-400 to-coral-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-coral-600 font-medium mb-1">
            ...
            <div className={`${
              ...
              {selectedConversation ? (
                <>
                  <div className="p-4 border-b border-slate-200 bg-cream-50">
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <button className="p-2 text-coral-600 hover:bg-white rounded-lg transition-colors">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3">
                    {messages.map(message => (
                      <div
                        key={message.id}
                        className={`flex items-end gap-2 ${message.senderType === 'patient' ? 'justify-end' : 'justify-start'}`}
                      >
                        {/* Provider Avatar */}
                        {message.senderType === 'provider' && (
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-coral-400 to-coral-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
                            ...
                            <div className="mt-2 space-y-1">
                              {message.attachments.map((file, index) => (
                                <div key={index} className={`text-xs ${
                                  message.senderType === 'patient' ? 'text-coral-100' : 'text-slate-500'
                                }`}>
                                  📎 {file}
                                </div>
                              ))}
                        ...
                        {/* Patient Avatar */}
                        {message.senderType === 'patient' && (
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-slate-400 to-slate-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
                  ...
                  <div className="p-3 border-t border-slate-200 bg-white">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 px-4 py-2.5 bg-slate-100 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-coral-500 focus:bg-white transition-colors"
                      />
                      <button
                        onClick={handleSendMessage}
                        className="p-2.5 bg-coral-500 text-white rounded-full hover:bg-coral-600 transition-colors shadow-sm"
                      >
                ...
                <div className="flex-1 flex items-center justify-center bg-cream-50">
                  <div className="text-center p-8">
                    <div className="w-20 h-20 bg-coral-50 rounded-full flex items-center justify-center mx-auto mb-4">
                    ...
                    <p className="text-slate-900 font-semibold text-lg">No conversation selected</p>
                    <p className="text-sm text-slate-600 mt-2">Choose a conversation from the left to start messaging</p>
                  </div>
                </div>
              )}
            </div>
